...
// ignore: 忽略属性
SgfNode.prototype.toSgf = function(ignore) {
...
SgfNode.prototype.getComment = function() {
	let c = this.getProperty(SgfProperty.C)
	if(c){
		return c[0]
	}
}
...
export function SgfMoveNode(col , row , color) {
	SgfNode.call(this);
	this.col = col;
	this.row = row;
	this.color = color;
...
SgfTree.prototype.autoPlay = function(col,row,color){
	...
	let result = {status:AutoPlayStatus.OK}
...
SgfTree.fromInt = function(c) {
	if(SgfTree.COLOR_BLACK === c) {
		return "B";
	}
	if(SgfTree.COLOR_WHITE === c) {
		return "W";
	}
}
